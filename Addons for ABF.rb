...
module ABFAddons
  ...
  # --- Константы ---
  CURRENT_VERSION = "1.4.0"  # !!! Текущая версия вашего плагина !!!
  ...
  FILES_TO_SKIP = [
    ...
    "Addons for ABF/prices.json",  # Пример. Добавьте СЮДА пути ко всем вашим JSON, если они есть.
    # "Addons for ABF/settings/another_settings_file.json"  <-- Раскомментируйте и добавьте, если будут другие
  ].freeze
  ...
  def self.check_for_updates
    begin
      ...
      if response.is_a?(Net::HTTPSuccess)
        ...
        UI.messagebox("Ошибка при проверке обновлений: #{response.code} - #{response.message}", MB_OK, "Addons for ABF")
      end

    rescue StandardError => e
      UI.messagebox("Ошибка при проверке обновлений: #{e.message}", MB_OK, "Addons for ABF")
    end
  end
    ...
    def self.show_update_dialog(new_version, data)
        dialog = UI::HtmlDialog.new(
          {
            :dialog_title => "Обновление Addons for ABF",
        ...
        <html>
        <head>
          <title>Обновление плагина</title>
          <style>
        ...
        <body>
          <div class="container">
            <p>Найдена новая версия: #{new_version}</p>
            <div class="button-container">
                <button onclick="sketchup.on_update()">Обновить</button>
                <button onclick="sketchup.on_close()">Закрыть</button>
            </div>
          </div>
        </body>
        </html>
  ...
  def self.install_update(data)
    begin
      ...
      UI.messagebox("Обновление успешно установлено. Перезапустите SketchUp для применения изменений.", MB_OK, "Addons for ABF")
       # Sketchup.reload_extension(ext) # Перезагрузка расширения

    rescue StandardError => e
      UI.messagebox("Ошибка при установке обновления: #{e.message}", MB_OK, "Addons for ABF")
    end
  end
  ...
  unless file_loaded?(__FILE__)
    ...
    submenu.add_item("Проверить обновления") { check_for_updates }  # Добавлено
    ...
    file_loaded(__FILE__)
     ...
     # Проверяем обновления при запуске (по желанию)
    Sketchup.add_observer(ABFAddons::StartupObserver.new)
  end

  # --- Startup Observer ---
  class StartupObserver < Sketchup::AppObserver
    def onStartupCompleted
      ABFAddons.check_for_updates
    end
  end
end