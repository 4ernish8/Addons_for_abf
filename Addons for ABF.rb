...
module ABFAddons
  ...
  CHECKED_FOR_UPDATES = false # Додаєм флаг

  # --- Список файлів що не чіпати ---
  FILES_TO_SKIP = [
  ...
  def self.check_for_updates
    # Чи не було вже перевірки в цьому сеансі
    ...
    # УСтавимо флаг що перевірка була
    @checked_for_updates = true
  end

  # --- Диалог оновлення (HTML) ---
  ...
  def self.install_update(data)
    begin
      data['files'].each do |file_info|
        ...
        # !!! Перевірка чи треба цей файл пропустити !!!
        next if FILES_TO_SKIP.include?(file_info['path'])

        # Створюємо каталоги якщо їх немає
        dir = File.dirname(local_path)
        Dir.mkdir(dir) unless Dir.exist?(dir)

        # Завантажуєм та зберігаємо файл
  ...
  unless file_loaded?(__FILE__)
    ...
    ext.description = 'Перенос кріпленнь та пазів з деталей'
    ext.version = version
    ext.creator = author
    ext.copyright = 'Олександр_К © 2024'
    Sketchup.register_extension(ext, true)

    file_loaded(__FILE__)

    # --- Таймер перевірки оновленнь---
